		...
		if (Perm.hasPermission(cs, Perm.CLAIM_ADMIN, Perm.CLAIM_PLUGIN)) {
			...
			if (cs instanceof Player) {
								...
								Lang.buildTC(
										Lang.PLUGIN_RADIUS.get().replace(VALUE,
												c.hasValue(Config.REGION_RADIUS) ? c.getValue(Config.REGION_RADIUS).toString()
														: "-"),
										null, Lang.PLUGIN_HOVER_RADIUS.get(), "/claim set " + CmdSet.CMD_RADIUS + " "));
				// Gap
				p.spigot()
						.sendMessage(
								Lang.buildTC(
										Lang.PLUGIN_GAP.get().replace(VALUE,
												c.hasValue(Config.REGION_GAP) ? c.getValue(Config.REGION_GAP).toString()
														: "-"),
										null, Lang.PLUGIN_HOVER_GAP.get(), "/claim set " + CmdSet.CMD_GAP + " "));
				// Expand Vert
				p.spigot()
						.sendMessage(Lang.buildTC(
								...
								"/claim set " + CmdSet.CMD_EXPAND_VERT + " " + (c.hasValue(Config.REGION_EXPAND_VERT)
				...
				p.spigot()
						.sendMessage(
								Lang.buildTC(
										Lang.PLUGIN_PATTERN_ID.get().replace(VALUE,
												c.hasValue(Config.REGION_PATTERN) ? c.getValue(Config.REGION_PATTERN).toString()
														: "-"),
										null, Lang.PLUGIN_HOVER_PATTERN_ID.get(), "/claim set " + CmdSet.CMD_PATTERN + " "));
				// Time Pattern
				p.spigot()
						.sendMessage(
								Lang.buildTC(
										Lang.PLUGIN_PATTERN_TIME.get().replace(VALUE,
												c.hasValue(Config.TIME_PATTERN) ? c.getValue(Config.TIME_PATTERN).toString()
														: "-"),
										null, Lang.PLUGIN_HOVER_PATTERN_TIME.get(), "/claim set " + CmdSet.CMD_TIME + " "));
				// Flags
