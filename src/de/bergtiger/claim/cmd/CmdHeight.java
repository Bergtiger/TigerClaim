            ...
            if (cs instanceof Player player) {
                if (cs instanceof Player p) {
                    ...
                    if (args.length >= 3) {
                        ...
                        // Nicht genügend Argumente angegeben
                        p.spigot().sendMessage(Lang.build("Um die Minimal- und Maximalhöhe einer Region zu verändern, " +
                                "gib bitte noch zuerst die Minimal- und danach die Maximalhöhe an."));
                        return;
                    }
                    if (!regionAngegeben) {
                    ...
                    if (newMinHeight != null && newMaxHeight != null) {
                        if (newMinHeight > newMaxHeight) {
                            p.spigot().sendMessage(Lang.build("Die angegebene Minimalhöhe (" + newMinHeight + ") ist größer als die angegebene Maximalhöhe (" + newMaxHeight + ")."));
                            return;
                        }
                        if (newMinHeight < player.getWorld().getMinHeight() && newMaxHeight > player.getWorld().getMaxHeight()) {
                            p.spigot().sendMessage(Lang.build("Die angegebene Minimalhöhe (" + newMinHeight + ") ist kleiner als die minimale Höhe der Welt (" +
                        ...
                        if (newMinHeight < player.getWorld().getMinHeight()) {
                            p.spigot().sendMessage(Lang.build("Die angegebene Minimalhöhe (" + newMinHeight + ") ist kleiner als die minimale Höhe der Welt (" + player.getWorld().getMinHeight() + ")."));
                            return;
                        }
                        if (newMaxHeight > player.getWorld().getMaxHeight()) {
                            p.spigot().sendMessage(Lang.build("Die angegebene Maximalhöhe (" + newMaxHeight + ") ist größer als die maximale Höhe der Welt (" + player.getWorld().getMaxHeight() + ")."));
                            return;
                        }

                        ConfirmationListener.inst().addConfirmation(new AdjustHeightsQueue(region, p.getWorld(), player, newMinHeight, newMaxHeight));
                        // inform Player
                        if (regionAngegeben) {
                            p.spigot().sendMessage(Lang.build("Möchtest du von der angegebenen Region die minimale Höhe von " + region.getMinimumPoint().getY() +
                            ...
                            p.spigot().sendMessage(Lang.build("Möchtest du von der Region, in der du stehst, die minimale Höhe von " + region.getMinimumPoint().getY() +
                                            " auf " + newMinHeight + " und die maximale Höhe von " + region.getMaximumPoint().getY() + " auf " + newMaxHeight + " setzen?"),
                                    Lang.build(Lang.HEIGHT_YES, "/yes", null, null),
                                    Lang.build(Lang.HEIGHT_NO, "/no", null, null));
                        }
                        ...
                        if (newMinHeight == null && newMaxHeight == null) {
                            p.spigot().sendMessage(Lang.build(args[1 + argumentenVerschiebung] + " und " + args[2 + argumentenVerschiebung] + " sind keine gültigen Zahlen."));
                            return;
                        }
                        if (newMinHeight == null) {
                            p.spigot().sendMessage(Lang.build(args[1 + argumentenVerschiebung] + " ist keine gültig Zahl."));
                            return;
                        }
                        if (newMaxHeight == null) {
                            p.spigot().sendMessage(Lang.build(args[2 + argumentenVerschiebung] + " ist keine gültig Zahl."));
                            return;
                        }
                    }
                } else {
